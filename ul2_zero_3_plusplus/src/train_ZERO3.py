...
class DeepSpeedMemoryFlushCallback(TrainerCallback):
    def __init__(self, interval=5):
        self.interval = interval   # flush every N steps
...
def main():
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        deepspeed=config['deepspeed']['deepspeed_zero3_plusplus_config'] if config['deepspeed']['enabled'] else None,
    ...
    # DeepSpeedMemoryFlushCallback(interval=5) Callback (optional)
    if config['training'].get("deepspeedMemoryFlush", False):
        cacheMemoryFlush_callback = DeepSpeedMemoryFlushCallback(interval=5)
        callbacks.append(cacheMemoryFlush_callback)
        if rank == 0:
            print("deepspeed cache Memory Flush  enabled")
    ...
    # ✅✅✅ CRITICAL: Explicitly disable use_cache BEFORE enabling gradient checkpointing
    if hasattr(model.config, 'use_cache'):
        model.config.use_cache = False

    # ✅ Enable gradient checkpointing with proper settings
    if config['training'].get("gradient_checkpointing", True):
        model.gradient_checkpointing_enable()

        # ✅ For T5/UL2 models, need to enable input gradients
        if hasattr(model, 'enable_input_require_grads'):
            model.enable_input_require_grads()

        if rank == 0:
            print("✅ Gradient checkpointing enabled")
            print("   - use_cache set to False")
    ...
    if rank == 0:
        ...
        print(f"  use_cache: {model.config.use_cache if hasattr(model.config, 'use_cache') else 'N/A'}")
        print(f"  gradient_checkpointing: {model.is_gradient_checkpointing if hasattr(model, 'is_gradient_checkpointing') else 'N/A'}")
        print()
    ...
    # Save model
    if rank == 0:
        print("zero3 based manual Saving model disabled and saving it using all ranks...")
        #trainer.save_model(config['paths']['trained_model_path'])
        #tokenizer.save_pretrained(config['paths']['trained_model_path'])
    
    model_dir = config['paths']['trained_model_path']
    ...
    try:
        ...
        # Path to consolidated model
        consolidated_path = os.path.join(model_dir, "ul2_model.bin")

        if not os.path.exists(consolidated_path):
            print(f"Converting checkpoint to FP32...")
            convert_zero_checkpoint_to_fp32_state_dict(
                model_dir,
                consolidated_path,
                tag=None  # Uses the latest checkpoint
            )
            print(f"✅ Consolidated model saved to: {consolidated_path}")
        else:
            print(f"✅ Consolidated model already exists: {consolidated_path}")

    except Exception as e:
        print(f"⚠️  Could not convert checkpoint: {e}")
        print(f"   You can convert later using: python zero_to_fp32.py {model_dir} {consolidated_path}")
    """
